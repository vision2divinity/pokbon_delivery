 ...
 *   expires offers that already exist and cascades from them, so a job with no
 *   offer was never revisited and sat at CREATED until somebody opened the Jobs
 *   page and pressed a second button. Order #87684 sat unoffered for 17
 *   minutes while the rider toggled duty and signed in and out, seeing nothing,
 *   because there was nothing to see. Jobs are now offered as they are created,
 *   on both paths. When nobody can be offered — every rider off duty is the
 *   ordinary case at night — the order gets a note saying so in words, because
 *   a job that looks dispatched and never moves is the failure this had.
...
define( 'POKBON_DELIVERY_VERSION', '0.5.27' );
